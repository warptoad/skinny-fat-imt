...
import { SkinnyIMTSha256, SkinnyIMTSha256Test } from "../typechain-types"
...
describe("SkinnyIMT", () => {
    let skinnyIMTTest: SkinnyIMTSha256Test
    let skinnyIMT: SkinnyIMTSha256
    let jsLeanIMT: JSLeanIMT

    beforeEach(async () => {
        const { library, contract } = await run("deploy:imt-sha256-test", {
        ...
        skinnyIMT = library
        jsLeanIMT = new JSLeanIMT((a, b) => sha256Hash(a, b))
    })
    ...
    describe("# precomputeRepeatedCache", () => {
        ...
        it("Should burn worst-case gas for insertManyRepeated in one call (every shortcut defeated)", async () => {
            ...
            //   * D sideNode SSTOREs
            // For D=255 that's ~508 sha256 calls vs ~0 in the old "best case".
            const D = 255n
        ...
        it("Should not change the root of a subsequent insertManyRepeated", async () => {
            ...
            const { contract: cold } = await run("deploy:imt-sha256-test", {
                library: "SkinnyIMTSha256",
                logs: false
            })
        ...
        it("Should make a follow-up insertManyRepeated cheaper", async () => {
            ...
            const { contract: warm } = await run("deploy:imt-sha256-test", {
                library: "SkinnyIMTSha256",
                logs: false
            })
