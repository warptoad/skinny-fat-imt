...
// VENDORED + PATCHED from poseidon2-evm@1.0.0 (zemse) src/Poseidon2Yul.sol.
//
// The upstream Poseidon2Yul omits modular reductions in its linear layers and
// overflows 2**256 on certain input-dependent states, producing wrong hashes
// (e.g. hash(13,14)). This copy reduces every linear-layer output < PRIME:
//   * internal_m_multiplication: addmod the sum accumulation and each output
//   * matrix_multiplication_4x4: addmod the two dirty outputs (t4 -> state3, t5 -> state1)
// Verified against @zkpassport/poseidon2 over 3263 pairs (incl. the upstream
// failing cases, a full lean-tree build, random field elements, and edges).
// Raw-fallback interface is unchanged: send tightly-packed inputs, no selector.

pragma solidity >=0.6.0;

contract Poseidon2YulFixed {
    fallback() external {
        assembly {
