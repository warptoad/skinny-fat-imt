...
contract SkinnyIMTPoseidonBareTest {
    SkinnyIMTData internal data;

    function insert(uint256 leaf) external {
        SkinnyIMTPoseidonBare.insert(data, leaf);
    }

    function insertMany(uint256[] calldata leaves) external {
        SkinnyIMTPoseidonBare.insertMany(data, leaves);
    }

    function insertManyRepeated(uint256 value, uint256 amount) external {
        SkinnyIMTPoseidonBare.insertManyRepeated(data, value, amount);
    }

    function update(uint256 oldLeaf, uint256 newLeaf, uint256 index, uint256[] calldata siblingNodes) external {
        SkinnyIMTPoseidonBare.update(data, oldLeaf, newLeaf, index, siblingNodes);
    }
    ...
    function updateMany(
        ...
        SkinnyIMTPoseidonBare.updateMany(data, oldLeaves, newLeaves, leafIndexes, proofSiblings);
    }

    function root() public view returns (uint256) {
        return SkinnyIMTPoseidonBare.root(data);
    }
