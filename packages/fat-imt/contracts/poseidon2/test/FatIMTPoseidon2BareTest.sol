...
contract FatIMTPoseidon2BareTest {
    FatIMTData internal data;

    function insert(uint256 leaf) external {
        FatIMTPoseidon2Bare.insert(data, leaf);
    }
    function insertMany(uint256[] calldata leaves) external {
        FatIMTPoseidon2Bare.insertMany(data, leaves);
    }
    function insertManyRepeated(uint256 value, uint256 amount) external {
        FatIMTPoseidon2Bare.insertManyRepeated(data, value, amount);
    }
    function update(uint256 newLeaf, uint256 index) external {
        FatIMTPoseidon2Bare.update(data, newLeaf, index);
    }
    function updateMany(uint256[] calldata newLeaves, uint256[] calldata leafIndexes) external {
        FatIMTPoseidon2Bare.updateMany(data, newLeaves, leafIndexes);
    }
    function root() public view returns (uint256) {
        return FatIMTPoseidon2Bare.root(data);
    }
