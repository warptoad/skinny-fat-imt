...
describe("FatIMTReadableStorage (multi-tree)", () => {
    ...
    it("Should clear a tree's leaves array on reset, then rebuild with no stale leaves", async () => {
        ...
        expect(await contract.getFatLeaves(TREE_A, 0, 0)).to.deep.equal([])
        ...
        expect(await contract.getFatLeaves(TREE_A, 0, 2)).to.deep.equal([77n, 88n])
        // the array is exactly length 2 now: reading past it reverts
        await expect(contract.getFatLeaves(TREE_A, 0, 3)).to.be.reverted
    ...
    it("Should read each tree's nodes independently (level 0 == leaves)", async () => {
        ...
        expect(await contract.getFatNodes(TREE_B, 0, leavesB.length, 0)).to.deep.equal(leavesB)
